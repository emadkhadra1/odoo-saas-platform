...
class BoqBusinessItemLine(models.Model):
    ...
    margin = fields.Float(string="هامش الربح",  required=False)
...
class BoqBusinessItem(models.Model):
    ...
    sub_item_id = fields.Many2one("b2b.sub.items",related="sub_business_statement_id.sub_item_id", string="البند الفرعي", required=True)
    main_item_id = fields.Many2one(related="sub_business_statement_id.main_item_id", string="البند الرئيسي", readonly=True, store=True)
    ...
    creation_date = fields.Date(string="تاريخ الإنشاء", required=True)

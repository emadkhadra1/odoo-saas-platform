...
class DeliveryPricingRule(models.Model):
    ...
    pricing_type = fields.Selection([
        ...
        ('subcontract_fee', 'Subcontractor Management Fee (رسوم إدارة المقاول من الباطن)'),
    ...
    # ── Subcontract Fee Fields ─────────────────────────────────────────────────
    subcontract_fee_pct = fields.Float(
        string='Management Fee % (نسبة رسوم الإدارة)',
        digits=(5, 2),
        help='Percentage of gross rider earnings deducted as management fee for subcontract riders.\n'
             'Example: 10 = deduct 10%% of gross amount.\n'
             'If both percentage and base_amount are set, percentage takes priority.',
        tracking=True,
    )
    subcontract_fee_basis = fields.Selection([
        ('gross', 'Gross Earnings (إجمالي الأرباح)'),
        ('order_base', 'Order Base Only (أساس الطلبات فقط)'),
        ('flat', 'Flat Monthly Amount (مبلغ ثابت شهري)'),
    ], string='Fee Basis (أساس الرسوم)', default='gross', tracking=True,
        help='Determines how the subcontract management fee is calculated:\n'
             '- Gross: fee_pct of total rider gross amount\n'
             '- Order Base: fee_pct of order_base_amount only\n'
             '- Flat: fixed base_amount deducted per month regardless of performance')

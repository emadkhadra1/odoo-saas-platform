            ...
            if rec.customer_rank or rec.supplier_rank:
                if rec.phone:
                    customers = self.env['res.partner'].search(
                        [('id', '!=', rec.id), '|', ('phone', '=', rec.phone), '|', ('mobile', '=', rec.phone), ('mobile2', '=', rec.phone)])
                ...
                if rec.mobile:
                    customers = self.env['res.partner'].search(
                        [('id', '!=', rec.id), '|', ('mobile', '=', rec.mobile), '|', ('phone', '=', rec.mobile), ('mobile2', '=', rec.mobile)])
                    customers = customers.filtered(lambda x: x.supplier_rank or x.customer_rank)
                    if customers:
                        raise exceptions.ValidationError('This number:{} repeated on another partner'.format(rec.mobile))
                if rec.mobile2:
                    customers = self.env['res.partner'].search(
                        [('id', '!=', rec.id), '|', ('mobile', '=', rec.mobile2), '|', ('phone', '=', rec.mobile2), ('mobile2', '=', rec.mobile2)])
                    customers = customers.filtered(lambda x: x.supplier_rank or x.customer_rank)
                    if customers:

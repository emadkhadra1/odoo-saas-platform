    ...
    def confirm(self):
        ...
        for line in self.receive_order_ids:
            ...
            if line.qty_available < line.qty:
                raise UserError(_('خطأ! الكمية غير متاحة للاعتماد.'))
            if not line.accept:
                raise ValidationError(_('خطأ! يوجد بند غير معتمد.'))

                # raise UserError(_('خطأ! يوجد بند غير معتمد.'))
            ...
            if self.location_id and self.location_dest_id and self.picking_type_id:
                ...
                for line in self.receive_order_ids:
                    if line.accept:
                        ...
                        #     error=''
                        #     raise UserError(_('خطأ! لا يمكن اعتماد التوريد.'))
                        # print origin
                        # print line
                    else:
...
class StockPicking(models.Model):
    ...
    change_receive_order = fields.Char(string="حالة أمر الاستلام",compute="_compute_change_receive_order", required=False, )
    receive_order_line_id = fields.Many2one(comodel_name="construction.receive.order.line",    string="بند الأمر", required=False, )
    #zakaria start
            ...
            for ops in pick.move_line_ids.filtered(lambda x: not x.move_id):
                ...
                if moves:
                    ...
                    new_move = self.env['stock.move'].create({
                                                    'name': _('حركة جديدة:') + ops.product_id.display_name,

...
class construction_project(models.Model):
    ...
    show_on_dashboard = fields.Boolean(string='إظهار اليومية في لوحة البيانات', help="تحديد ما إذا كانت اليومية ستظهر في لوحة البيانات أم لا", default=True)
    ...
    def get_bar_graph_datas(self):
        ...
        data.append({'label': _('سابق'), 'value':0.0, 'type': 'past'})
    ...
    def get_journal_dashboard_datas(self):
        ...
        if self.type in ['bank', 'cash']:
            ...
            account_ids = tuple(filter(None, [self.default_account_id.id, self.default_account_id.id]))
            if account_ids:
        ...
        #TODO need to check if all invoices are in the same currency than the journal!!!!
            ...
            title = _('فواتير مستحقة الدفع') if self.type == 'purchase' else _('فواتير مستحقة لصالحك')
    ...
    def action_create_new(self):
        ...
        model = 'account.move'
        if self.type == 'sale':
            ctx.update({'journal_type': self.type, 'default_move_type': 'out_invoice', 'type': 'out_invoice', 'default_journal_id': self.id})
            if ctx.get('refund'):
                ctx.update({'default_move_type':'out_refund', 'type':'out_refund'})
            view_id = self.env.ref('account.invoice_form').id
        elif self.type == 'purchase':
            ctx.update({'journal_type': self.type, 'default_move_type': 'in_invoice', 'type': 'in_invoice', 'default_journal_id': self.id})
            if ctx.get('refund'):
                ctx.update({'default_move_type': 'in_refund', 'type': 'in_refund'})

    ...
    () => {
        ...
        if (window.innerWidth >= 992 && prevValue < 992) {
			if (document.querySelector('body').classList.contains('horizontal')) {
				...
				// first level
				var parent = $("[data-toggle='sub-slide']").parents('ul');
                var ul = parent.find('ul:visible').slideUp(animationSpeed);
                ul.removeClass('open');
                var parent1 = $("[data-toggle='sub-slide2']").parents('ul');
...
function menuClick() {
	$("[data-toggle='slide']").off('click');
	$("[data-toggle='sub-slide']").off('click')
	$("[data-toggle='sub-slide2']").off('click')
